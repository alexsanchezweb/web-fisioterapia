      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {!hideHeader && (
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-bold text-[var(--color-primary)] mb-5 leading-tight">
              ...
              <span className="text-[var(--color-secondary)]">Sin sorpresas.</span>
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          {plans.map((plan, i) => (
            ...
            >
              {plan.popular && (
                <div className="bg-[var(--color-primary)] px-8 py-3 flex items-center justify-between">
              ...
              <div className="bg-white p-8">
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-[var(--color-primary)] mb-2">{plan.name}</h3>
                ...
                <div className="mb-8 pb-8 border-b border-gray-100">
                  <div className="flex items-baseline gap-1.5 flex-wrap">
                    ...
                    <span className="text-5xl font-bold text-[var(--color-primary)]">€{plan.price}</span>
                    <span className="text-gray-400 text-sm">{plan.period}</span>
                  </div>
                  {plan.originalPrice && (
                    <p className="text-[var(--color-secondary)] text-xs font-semibold mt-1.5">
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-3 text-sm">
                      <svg className="w-5 h-5 text-[var(--color-secondary)] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a
                  ...
                  className={`block w-full py-3.5 rounded-lg font-semibold text-sm text-center transition-all duration-200 ${
                    plan.popular
                      ? "bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white shadow-sm hover:shadow-md"
