...
const waText = encodeURIComponent("Hola, me gustaría reservar una consulta");
...
function buildInfo() {
  return [
    ...
    },
    {
      ...
      title: "Horario",
      text: `${config.scheduleWeekdays}\n${config.scheduleSaturday}`,
    },
    {
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Contact({ hideHeader = false }: ContactProps) {
  ...
  const inputClass =
    "w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-gray-800 placeholder-gray-400 text-sm focus:outline-none focus:border-[var(--color-primary)]/40 focus:ring-1 focus:ring-[var(--color-primary)]/20 transition-all duration-200 font-light";
        ...
        {!hideHeader && (
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-bold text-[var(--color-primary)] mb-5 leading-tight">
              Dé el primer paso{" "}
              <span className="text-[var(--color-secondary)]">hoy</span>
            </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
            {sent ? (
              <div className="text-center py-12">
                <div className="w-16 h-16 rounded-full bg-[var(--color-secondary-light)] border border-[var(--color-secondary-border)] flex items-center justify-center mx-auto mb-5">
                  <svg className="w-7 h-7 text-[var(--color-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <h3 className="text-xl font-semibold text-[var(--color-primary)] mb-2">Mensaje recibido</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-4 bg-[var(--color-secondary)] hover:bg-[var(--color-secondary-dark)] disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold text-sm rounded-lg transition-all duration-200 shadow-sm hover:shadow-md tracking-wide"
                >
          ...
          <div className="space-y-3">
            {info.map((item, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-lg bg-[var(--color-secondary-light)] text-[var(--color-secondary)] flex items-center justify-center flex-shrink-0">
                  {item.icon}
                </div>
                <div>
                  <p className="text-sm font-semibold text-[var(--color-primary)] mb-1">{item.title}</p>
            ...
            <a
              ...
              className="flex items-center gap-3 p-5 rounded-xl bg-white border border-gray-200 hover:border-[var(--color-secondary)]/30 hover:bg-[var(--color-secondary-light)] transition-all duration-200 shadow-sm"
            >
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--color-primary)]">Escríbanos por WhatsApp</p>
